...
class WC_Gateway_Khalti extends WC_Payment_Gateway
{
    ...
    public function is_valid_for_use()
    {
        return in_array(
            ...
            apply_filters(
                'woocommerce_khalti_supported_currencies',
                array('NPR')
            ),
            true
        );
    ...
    public function admin_options()
    {
        if ($this->is_valid_for_use()) {
            ...
            $error_notice .= '<strong>' . esc_html_e(
                'Gateway Disabled',
                'woocommerce-khalti'
            ) . ':</strong>';
            $error_notice .= esc_html_e(
                'Khalti does not support your store currency.',
                'woocommerce-khalti'
            );
            $error_notice .= '</p></div>';

            echo wp_kses_post($error_notice);
        }
    }
    ...
    public function admin_scripts()
    {
        ...
        wp_enqueue_script(
            'wc-khalti-admin',
            plugins_url(
                '/assets/js/khalti-admin' . $suffix . '.js',
                WC_KHALTI_PLUGIN_FILE
            ),
            array(),
            '',
            true
    ...
    public function public_styles()
    {
        wp_enqueue_style(
            'wc-khalti-public',
            plugins_url(
                '/assets/css/khalti-public-style.css',
                WC_KHALTI_PLUGIN_FILE
            ),
            array(),
            '',
